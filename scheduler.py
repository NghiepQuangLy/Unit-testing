...
class Scheduler:
    ...
    def find_time(self, satlist_url="http://celestrak.com/NORAD/elements/visual.txt", start_time=datetime.now(), n_windows=24, duration=60, sample_interval=1, cumulative=False, location=(-37.910496, 145.134021)):
        ...
        # start_time_max_sub_interval, visible_satellites_max_sub_interval = self.find_max_visible_satellites_interval_non_cumulative(
        #     satellites_list, observer_location, start_time, duration, sample_interval)

        # print("There are at maximum {} visible satellites".format(len(visible_satellites_max_sub_interval)))

        # for visible_satellite in visible_satellites_max_sub_interval:
        #     print(visible_satellite.name)

        # print("The start time is {}".format(start_time_max_sub_interval))

        # start_time_interval, visible_satellites_interval = self.find_max_visible_satellites_interval_cumulative(
        #     satellites_list, observer_location, start_time, duration, sample_interval)

        # print("We have {} visible satellites in this interval and they are:".format(len(visible_satellites_interval)))

        # for visible_satellite in visible_satellites_interval:
        #     print(visible_satellite.name)

        # print("Time of interval is {}".format(start_time_interval))
        # print("We have {} in total".format(len(satellites_list)))

        # TODO: integrate n_windows with below functions
        if cumulative:
            start_time_interval, visible_satellites_interval = self.find_max_visible_satellites_interval_cumulative(
                satellites_list, observer_location, start_time, duration, sample_interval)
            ...
            return start_time_max_sub_interval, visible_satellites_max_sub_interval

    @staticmethod
    def get_all_satellites(satellite_list_url='http://celestrak.com/NORAD/elements/visual.txt'):
        """
...
if __name__ == "__main__":
    ...
    interval, satellites = a.find_time(cumulative=False)
    ...
    cumulative_interval, cumulative_satellites = a.find_time(cumulative=True)

    print("\nCumulative Interval: {}".format(cumulative_interval))
    for satellite in cumulative_satellites:
        print(satellite.name)
